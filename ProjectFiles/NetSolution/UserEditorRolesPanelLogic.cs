...
public class UserEditorRolesPanelLogic : BaseNetLogic
{
    ...
    private void BuildUIRoles()
    {
        ...
        foreach (var role in roles.Children)
        {
            ...
            if (editable.Value)
            {
                roleUiObject = InformationModel.MakeObject<Panel>(role.BrowseName, training_prague.ObjectTypes.GroupCheckbox);
            }
            else if (UserHasRole(role.NodeId))
            {
                roleUiObject = InformationModel.MakeObject<Panel>(role.BrowseName, training_prague.ObjectTypes.GroupLabel);
            }
